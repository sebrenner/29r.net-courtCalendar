...
class PDF extends FPDF
{
	...
	function Header()
	{
	    ...
	    $month = date('F');
    	    $year = date ('Y');
	    $title = "Judge " . ucfirst( $this->judge ) . " - Civil Cases that will be over time as of $month $year.";
	    $memo = "These cases, if not resolved this month, will likely appear as overtime on the S. Ct. Form A for $month $year.";
	...
	function Footer()
	{
	    ...
	    $notes = "Cases listed on this report were filed ~18 months ago.";
	    $this->Ln();
	...
	function FancyTable( $header, $data )
	{
		...
		$this->Cell($w[0],$lineHeight,$row["case_number"],'L' . $border  ,0,'L',$fill);	            
	        $this->Cell($w[1],$lineHeight,ucwords( strtolower( substr( $row["caption"],0,27))),'L' . $border  ,0,'L',$fill);	            
...
class caseOvertimeList
{
    ...
    protected function getSentinelCaseNum( ){
    	...
    	$endOfSentinelDay = strtotime('-1 second', strtotime('-17 month', $result));
 	$startOfSentinelDay = strtotime('- 23 hours', $endOfSentinelDay );
    	
    	while ( true ){
	    	// Query Db for DSCs on $result with highest case number. If no DSC found look at 1 day earlier
	    	$query = "SELECT case_number FROM nextActions "
		    . " WHERE NAC like \"%CMC%\""
		    . " and NAC_date between \"" 
		...
		if( $result = mysql_unbuffered_query( $query )){
			while( $row = mysql_fetch_array( $result, MYSQL_ASSOC ) ){
				if ($row["case_number"] != null){
					//echo "returning: " . $row["case_number"];
					return $row["case_number"];
				};
			}
		}
		// Advance the start date until you find a CMC
 		$startOfSentinelDay = strtotime('-1 day', $startOfSentinelDay);
 		//echo "\n\n\nThe new startOfSentinelDay = $startOfSentinelDay\n\n";
	}
}
}
