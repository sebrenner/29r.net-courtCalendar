 ...
 * ical directory/file read/write error OR iCalcreator parse error will be directed to error_log
 *
 * @author Kjell-Inge Gustafsson <ical@kigkonsult.se>
 * @since 2.0 - 2009-03-27
 * @param string $filename      file to convert (incl. opt. directory)
 * @param array  $conf          map iCal property names to user friendly names, ex. 'DTSTART' => 'startdate'
 ...
 * @param string $diskfilename  opt, filename for file to save or else taken from $filename + 'xls' extension
 ...
 */
function iCal2xls( $filename, $conf=FALSE, $save=FALSE, $diskfilename=FALSE, $log=FALSE ) {
  if( $log ) $timeexec = array( 'start' => microtime( TRUE ));
  $iCal2xls_VERSION = 'iCal2xls 2.0';
  if( !function_exists( 'fileCheckRead' ))
    require_once 'fileCheck.php';
  if( !class_exists( 'vcalendar', FALSE ))
    require_once 'iCalcreator.class.php';
  require_once 'Spreadsheet/Excel/Writer.php';
  if( $log ) $log->log( "$iCal2xls_VERSION input=$filename, conf=".var_export($conf,TRUE).", save=$save, diskfilename=$diskfilename", 7 );
  ...
  foreach( $conf as $key => $value ) {
    ...
    elseif(( '2' <= $key) && ( '99' > $key )) {
      ...
      if( $log ) $log->log( "$iCal2xls_VERSION column $key contains ".strtoupper( $value ), 7 );
    }
    elseif( in_array( $key, array( 'del', 'sep', 'nl' )))
      $conf[$key] = "$value";
    else {
      $conf[strtoupper( $key )] = $value;
      if( $log ) $log->log( "$iCal2xls_VERSION ".strtoupper( $key )." mapped to $value", 7 );
    }
  }
  ...
  if( $remoteInput ) {
    ...
    if( !$diskfilename )
      $diskfilename = $inputFileParts['filename'].'.xls';
  }
  else {
    if( FALSE === ( $filename = fileCheckRead( $filename, $log ))) {
      if( $log ) {
        $log->log( "$iCal2xls_VERSION (".number_format(( microtime( TRUE ) - $timeexec['start'] ),  5 ).')' );
        $log->flush();
      }
      return FALSE;
    }
    ...
    if( !$diskfilename )
      $diskfilename = $inputFileParts['dirname'].DIRECTORY_SEPARATOR.$inputFileParts['filename'].'.xls';
  }
  ...
  if( $log ) {
    $msg = $iCal2xls_VERSION.' INPUT FILE:"'.$inputFileParts['dirname'].DIRECTORY_SEPARATOR.$inputFileParts['basename'].'"';
    if( $save )
  ...
  if( $remoteInput ) {
    if( FALSE === $calendar->setConfig( 'url', $filename )) {
      $msg = $iCal2xls_VERSION.' ERROR 3 INPUT FILE:"'.$filename.'" iCalcreator: invalid url';
    ...
    if( FALSE === $calendar->setConfig( 'directory', $inputFileParts['dirname'] )) {
      $msg = $iCal2xls_VERSION.' ERROR 4 INPUT FILE:"'.$filename.'" iCalcreator: invalid directory: "'.$inputFileParts['dirname'].'"';
      if( $log ) { $log->log( $msg, 3 ); $log->flush(); } else error_log( $msg );
      return FALSE;
    }
    if( FALSE === $calendar->setConfig( 'filename',  $inputFileParts['basename'] )) {
      $msg = $iCal2xls_VERSION.' ERROR 5 INPUT FILE:"'.$filename.'" iCalcreator: invalid filename: "'.$inputFileParts['basename'].'"';
      if( $log ) { $log->log( $msg, 3 ); $log->flush(); } else error_log( $msg );
      return FALSE;
    }
  }
  if( FALSE === $calendar->parse()) {
    $msg = $iCal2xls_VERSION.' ERROR 6 INPUT FILE:"'.$filename.'" iCalcreator parse error';
    if( $log ) { $log->log( $msg, 3 ); $log->flush(); } else error_log( $msg );
    return FALSE;
  }
  ...
  /* Creating a workbook */
  if( $save )
    $workbook = new Spreadsheet_Excel_Writer( $diskfilename );
  else
  ...
  foreach( $conf as $key => $value ) {
    if(( '2' <= $key) && ( '99' > $key )) {
      $proporder[$value] = $key;
      if( $log ) $log->log( "$iCal2xls_VERSION $value in column $key", 7 );
    }
  }
  $proporder['TYPE']          =  0;
  $proporder['ORDER']         =  1;
  ...
  foreach( $props as $prop ) {
    ...
    if( isset( $conf['skip'] ) && in_array( $prop, $conf['skip'] )) {
      if( $log ) $log->log( "$iCal2xls_VERSION $prop removed from output", 7 );
      continue;
    }
  ...
  if( $log ) $log->log( "$iCal2xls_VERSION zone proporder=".implode(',',array_flip($proporder)), 7 );
  ...
  /* create vtimezone header row */
    foreach( $proporder as $propName => $col ) {
      if( isset( $conf[$propName] )) {
        $worksheet->writeString( $row,  $col, $conf[$propName], $format_bold ); // check map of userfriendly name to iCal property name
        if( $log ) $log->log( "$iCal2xls_VERSION header row, col=$col: $propName, replaced by ".$conf[$propName], 7 );
      }
      else
      ...
      foreach( $proporder as $propName => $col ) {
        ...
        if( isset( $compinfo['props'][$propName] )) {
          ...
          if( !method_exists ( $comp, $fcn )) {
            $msg = $iCal2xls_VERSION.' ERROR 7 INPUT FILE:"'.$filename.'" iCalcreator: unknown property: "'.$propName.'" ('.$fcn.')';
          ...
          foreach( $proporder as $propName => $col ) {
            ...
            if( isset( $compinfo2['props'][$propName] )) {
              ...
              if( !method_exists ( $scomp, $fcn )) {
                $msg = $iCal2xls_VERSION.' ERROR 8 INPUT FILE:"'.$filename.'" iCalcreator: unknown property: "'.$propName.'" ('.$fcn.')';
                if( $log ) $log->log( $msg, 3 ); else error_log( $msg );
                continue;
              }
...
//  $potmp[2]                   =  'UID';
  ...
  ksort( $potmp, SORT_NUMERIC );
  $proporder = array_flip( array_values( $potmp ));
  if( $log ) $log->log( "$iCal2xls_VERSION comp proporder=".implode(',',array_flip($proporder)), 7 );
...
//    $worksheet->write(         $row, $proporder['UID'],   $compinfo['uid'] );
    ...
    foreach( $proporder as $propName => $col ) {
      ...
      if( isset( $compinfo['props'][$propName] )) {
        ...
        if( !method_exists ( $comp, $fcn )) {
          $msg = $iCal2xls_VERSION.' ERROR 9 INPUT FILE:"'.$filename.'" iCalcreator: unknown property: "'.$propName.'" ('.$fcn.')';
          if( $log ) $log->log( $msg, 3 ); else error_log( $msg );
          continue;
        }
        $output = str_replace( "$calnl ",     '',      rtrim( $comp->$fcn()));
      ...
      foreach( $compinfo['sub'] as $compinfo2 ) {
        ...
        foreach( $proporder as $propName => $col ) {
          ...
          if( isset( $compinfo2['props'][$propName] )) {
            $fcn = 'create'.strtoupper( substr( $propName, 0, 1 )).strtolower( substr( $propName, 1 ));
            if( !method_exists ( $scomp, $fcn )) {
              $msg = $iCal2xls_VERSION.' ERROR 10 INPUT FILE:"'.$filename.'" iCalcreator: unknown property: "'.$propName.'" ('.$fcn.')';
              if( $log ) $log->log( $msg, 3 ); else error_log( $msg );
              continue;
            }
            $output = str_replace( "$calnl ",     '',      rtrim( $scomp->$fcn()));
  ...
  if( $log ) {
    ...
    $log->log( $msg, 7 );
    $msg  = "$iCal2xls_VERSION '$filename' (".count($compsinfo).' components) start:'.date( 'H:i:s', $timeexec['start'] );
    $msg .= ' total:'  .number_format(( $timeexec['exit']    - $timeexec['start'] ),   5 ).'sec';
    if( $save )
      $msg .= " -> '$diskfilename'";
    $log->log( $msg, 6 );
